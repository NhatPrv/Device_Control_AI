...
async def execute_command(command: str):
    """
    Spawns a fresh Agent session with a unique conversation ID to execute the command.
    This guarantees no history pollution or tool looping from previous turns.
    """
    conv_id = str(uuid.uuid4())
    config = get_agent_config(conversation_id=conv_id)
...
async def main():
    ...
    try:
        # 3. Initialize STT with chosen language
        stt_manager = STTManager(language=selected_lang)
        
        # 4. Start communication loop
        if selected_lang == "vi":
